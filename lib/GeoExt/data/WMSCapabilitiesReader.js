    ...
    if(typeof recordType !== "function") {
        recordType = GeoExt.data.LayerRecord.create(
             [
                ...
                {name: "styles"}, // array
                {name: "srs"}, // object
                {name: "dimension", mapping: "dimensions.time.values", type: "object"}, // object
                {name: "bbox"}, // object
                {name: "llbbox"}, // array
...
Ext.extend(GeoExt.data.WMSCapabilitiesReader, Ext.data.DataReader, {
    ...
    readRecords: function(data) {
        ...
        //alert(data.capabilities.layers[2].dimension);
        ...
        if(url && layers) {
            ...
            for(var i=0, lenI=layers.length; i<lenI; i++){
                ...
                if(layer.name) {
                    if(i==1){
                        console.log(layer['dimensions']['time']);
                    }
                    values = {};
                    for(var j=0, lenJ=fields.length; j<lenJ; j++) {
                        ...
                        if(field.mapping){
                            ...
                            //console.log(field.mapping.indexOf("."));
                            //console.log(field.mapping.indexOf('\\.'));
                            if(field.mapping.indexOf('.') != -1){
                           // console.log(field.mapping.split('.'));
                            bits = field.mapping.split('.');
                            tempv = layer;
                            for( var p=0, lenp=bits.length; p<lenp; p++){
                                tempv = tempv[bits[p]];
                            }
                            console.log(tempv);
                            //v = layer[bits[0]][bits[1]][bits[2]];
                            v = tempv;
                            }
                        }
                        else {
